...
const MeditationTracker = () => {
    ...
    useEffect(() => {
        if(weeklyData.length > 0) {
            let td = new Array(7).fill(0);
            weeklyData.forEach(element => {
                const dayIndex = new Date(element.date).getDay();
                td[dayIndex] = Math.min(Math.ceil(element.activity.meditation), DAILY_GOAL_MINUTES * 60);
            });
            setTargetData(td);
        }
    }, [weeklyData]);
    ...
    const reset = () => {
        ...
        // Get today's current total from the targetData
        const currentDayTotal = newTargetData[day] || 0;
        // Add new session time
        const newDayTotal = currentDayTotal + totalSec;
        // Cap at daily goal (40 minutes = 2400 seconds)
        newTargetData[day] = Math.min(newDayTotal, DAILY_GOAL_MINUTES * 60);
        ...
        if (newDayTotal >= DAILY_GOAL_MINUTES * 60) {
            alert("Congratulations! You've completed your daily meditation goal of 40 minutes!");
        }
        ...
        >
            ...
            <Grid item xs={12}>
                <Card className="whiteBox targetCard">
                    <TargetChart color="#51ab55" data={targetData} goal={DAILY_GOAL_MINUTES * 60} />
                </Card>
            </Grid>
        </Grid>
    );   
